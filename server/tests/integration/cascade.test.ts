...
describe('Cascade Deletion Integrity', () => {
    it('Debe borrar un diagrama complejo y no dejar huérfanos', async () => {
        ...
        const tRes = await fetch(`${BASE_URL}/tables`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ id: `t-${runId}`, diagramId, name: 'T1', fields: [] })
        });
        ...
        const nRes = await fetch(`${BASE_URL}/notes`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ id: `n-${runId}`, diagramId, content: 'N1' })
        });
        ...
        const aRes = await fetch(`${BASE_URL}/areas`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ id: `a-${runId}`, diagramId, name: 'A1', x:0, y:0, width:10, height:10 })
        });
        expect(aRes.status).toBe(201);

        // 3. Borrar Diagrama
        const delRes = await fetch(`${BASE_URL}/diagrams/${diagramId}`, { method: 'DELETE' });
        expect(delRes.status).toBe(200);

        // 4. Verificación vía API
        await new Promise(resolve => setTimeout(resolve, 500));
        
        const tablesRes = await fetch(`${BASE_URL}/tables?diagramId=${diagramId}`);
        const tables = await tablesRes.json();
        expect(tables.length).toBe(0);

        const notesRes = await fetch(`${BASE_URL}/notes?diagramId=${diagramId}`);
        const notes = await notesRes.json();
        expect(notes.length).toBe(0);

        const areasRes = await fetch(`${BASE_URL}/areas?diagramId=${diagramId}`);
